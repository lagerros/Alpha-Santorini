...
def conv_block(features, f, activation_fn, phase, k):
    ...
    norm_conv = tf.layers.batch_normalization(conv, training=phase)
    if activation_fn != None:
        norm_conv = activation_fn(norm_conv)
    return norm_conv
...
class deep_net():
    ...
    def __init__(self, phase, scope, lr):
        ...
        with tf.variable_scope(scope):
            ...
            #policy head
            pre_p = conv_block(self.get_tower(height), 2, tf.nn.relu6, self.phase, 1)
